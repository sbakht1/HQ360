...
const district = window.location.href.includes('type=district');
let cols=[];

if((!district)){
  cols.push({ headerName:'Store',field: 'StoreName',filter:'agSetColumnFilter'})
}
...
cols.push(
  ...
  { headerName:'Gross Profit',field: 'gp', filter: 'agTextColumnFilter',
    cellRenderer: p => parseInt(p.value.replace(',','')).toLocaleString('en-US',{style:'currency',currency:'USD',minimumFractionDigits:0})
  },
  { 
  ...
  },
  {
    cellRenderer: params => {
      ...
      if (user === 'administrator') user = 'admin';
        return `
          <a data-toggle="modal" href="#scorecard" data-src="${window.app.url+'/'+user+'/emailsync/card/'+params.data.id}" class="btn btn-sm btn-primary sc-btn">View</a>
        `;
    }
  });
